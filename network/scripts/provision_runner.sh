...
BASIC_TOFU_DIR="network/infra/stackit-runner"
BASIC_TFVARS_FILE="network/infra/stackit-runner/basic-infra.tfvars"
SERVICE_ACCOUNT_JSON=""
ACCESS_MODE="private"

usage() {
  cat >&2 <<USAGE
usage: $0 [--scenario FILE ... | --scenario-dir DIR] [--benchmark NAME ...] [--destroy always|success|never] [--continue-on-error] --service-account-json PATH [--basic-tofu-dir DIR] [--basic-tfvars-file FILE] [--run-id ID]
USAGE
}
...
rsync -az --delete -e "$SSH_REMOTE_CMD" \
  ...
  --include '/network/' \
  --include '/network/***' \
  --exclude '/network/**/.terraform/' \
  --exclude '/network/**/terraform.tfstate*' \
  --exclude '/network/**/artifacts/' \
  --exclude '/network/**/logs/' \
  --exclude '/network/**/state/' \
  --exclude '/network/**/*.tfvars' \
  --exclude '/network/**/*.tfvars.json' \
  --exclude '*' \
  "${REPO_ROOT}/" \
  "${RUNNER_SSH_USER}@${RUNNER_PUBLIC_IP}:${RUNNER_WORKDIR}/"

log "copying service account key to the runner"
runner_ssh "$RUNNER_PUBLIC_IP" "mkdir -p '${RUNNER_WORKDIR}/state' '${RUNNER_WORKDIR}/bin' '${RUNNER_WORKDIR}/artifacts/network' '${RUNNER_WORKDIR}/repo'"
...
REMOTE_BASELINE_TFVARS="${RUNNER_WORKDIR}/network/scenarios/stackit-baseline.tfvars"
...
runner_ssh "$RUNNER_PUBLIC_IP" "cd '${RUNNER_WORKDIR}' && bash ./network/scripts/launch_runner_job.sh --run-id '${RUN_ID}' --destroy '${DESTROY_MODE}' --access-mode private -- $(shell_join "${remote_runner_args[@]}")"

log "runner public ip: ${RUNNER_PUBLIC_IP}"
log "runner private ip: ${RUNNER_PRIVATE_IP}"
log "status command:"
echo "ssh -i '${RUNNER_SSH_KEY}' ${RUNNER_SSH_USER}@${RUNNER_PUBLIC_IP} \"tail -f '${RUNNER_WORKDIR}/artifacts/network/${RUN_ID}/launcher.log'\""
log "fetch command:"
echo "${SCRIPT_DIR}/fetch_runner_results.sh --runner-host ${RUNNER_PUBLIC_IP} --ssh-key '${RUNNER_SSH_KEY}' --run-id '${RUN_ID}' --out '${LOCAL_OUT}'"
